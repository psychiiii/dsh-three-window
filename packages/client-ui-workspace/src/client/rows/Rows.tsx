...
/** Child-seat renderer threaded from the browser root for the row's leading decoration and hover-card section. */
type RowRenderSlot = WorkspaceBrowserProps['renderSlot']

const OCCUPANCY_KEYS = ['occupancy.chat', 'occupancy.construct', 'occupancy.review'] as const
const NEW_WINDOW_KEYS = ['windows.newWindow.chat', 'windows.newWindow.construct', 'windows.newWindow.review'] as const
const OCCUPANCY_CLASS = ['occupancyChat', 'occupancyConstruct', 'occupancyReview'] as const

function occupancyKey(index: 0 | 1 | 2): (typeof OCCUPANCY_KEYS)[number] {
  return OCCUPANCY_KEYS[index]
}

function occupancyClass(index: 0 | 1 | 2): string {
  return css[OCCUPANCY_CLASS[index]] as string
}
 ...
 * @param props.onActivate - three-window mode: the row names the Workspace the
 *   panes show, and only the separate chevron button expands or collapses.
 * @param props.focused - the panes show this Workspace.
 * @param props.onCreate - start a frontend Session inside this Workspace.
 * @param props.onCreateWindow - three-window mode: the ＋ button opens a pane
 *   menu and creates a window of the chosen kind here.
...
export function ProjectRowItem({
  ...
  const ownRow = (
    <div
      className={clsx(css.projectRow, (menuOpen || windowMenuOpen) && css.menuOpen, focused && css.projectFocused)}
      role="treeitem"
      aria-expanded={row.expanded}
      data-workbench-focused={focused ? 'true' : undefined}
    ...
    >
      ...
      {onActivate === undefined
        ...
        : (
          <button
            type="button"
            className={clsx(css.slot, css.expandButton)}
            aria-label={t('windows.expand.aria', { name: label })}
            aria-expanded={row.expanded}
            data-workbench-expand=""
        ...
        {onCreateWindow === undefined
          ...
          : (
            <Menu
              ...
              items={NEW_WINDOW_KEYS.map((key, index) => ({ id: String(index), label: t(key) }))}
              onSelect={(id) => {
                setWindowMenuOpen(false)
                const index = Number(id)
                /* v8 ignore next -- Menu emits only the three ids supplied above. */
                if (index !== 0 && index !== 1 && index !== 2) return
                onCreateWindow(index)
              }}
              portal
              closeOnPointerLeave
              anchor={(
...
export function SessionNodeItem({
  node, currentId, now, onOpen, onRename, onFork, onArchive, onUnarchive, onReveal, drag, flat = false, renderSlot, t,
}: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Archive this session (row menu action; commits without a dialog). Not offered on a current window. */
  onArchive: (id: SessionNode['id']) => void
  /** Unarchive this archived past window; its menu offers only this. */
  ...
  /** Renders the row's leading-decoration and hover-card child seats. */
  renderSlot: RowRenderSlot
  t: RowTranslate
}) {
  const row = node
  ...
  // confirmation dialog.
  // A current workbench window has no archive item: archiving it would empty
  // its pane. An archived past window offers only unarchive.
  const sessionMenuItems = node.archived === true
  ...
  const ownRow = (
    <div
      ref={rowRef}
      className={clsx(
        css.sessionRow, selected && css.selected, menuOpen && css.menuOpen, node.active === true && css.windowActive,
      ...
      data-workbench-active={node.active === true ? 'true' : undefined}
    ...
    >
      {/* Pending interaction and own or descendant activity outrank the
          ...
          and is cleared by opening the session. The leading seat takes the
          cell only while no status dot does, and never on an archived or
          blank row. */}
      {(!flat || showStatus) && (
        <span className={css.slot}>
          {showStatus
            ? <SessionStatusDots statuses={statuses} />
            : !row.archived && !row.blank && renderSlot('sidebar.session.row.leading', { sessionId: node.id })}
        </span>
      )}
      {node.occupancy !== undefined && (
        <span
          className={clsx(css.occupancy, occupancyClass(node.occupancy))}
          data-workbench-occupancy={String(node.occupancy)}
        >
          {t(occupancyKey(node.occupancy))}
        </span>
      )}
      {node.occupancy === undefined && node.past !== undefined && (
        <span
          className={clsx(css.occupancy, css.occupancyPast, occupancyClass(node.past))}
          data-workbench-past={String(node.past)}
        >
