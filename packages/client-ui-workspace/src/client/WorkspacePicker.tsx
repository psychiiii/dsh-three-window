...
export function WorkspacePicker({
  ...
  return (
    <WorkspacePickFlow
      ...
      selectedId={selectedId}
      // In three-window mode this chip names the Workspace the three panes
      // show, so picking one enters it. The owner's onPick would instead move
      // this one pane's blank Session across, splitting the row between two
      // Workspaces; it stays the behaviour wherever no workbench answers.
      onPick={(workspaceId) => {
        onClose()
        if (focusWorkspace(workspaceId)) return
        onPick(workspaceId)
      }}
      onClose={onClose}
    />
