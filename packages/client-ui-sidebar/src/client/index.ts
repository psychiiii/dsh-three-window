...
interface WindowStarter {
  windowed(): boolean
  createWindow(index: number): Promise<void>
}
...
export function apply(ctx: ClientContext): void {
  ...
  const windowedStore = createSnapshotStore(false)
  const syncWindowed = (): void => {
    const workbench = ctx.get('workbench') as WindowStarter | undefined
    const next = workbench?.windowed() === true
    if (windowedStore.getSnapshot() !== next) windowedStore.set(next)
  }
  ctx.inject(['workbench'], (ready) => {
    const workbench = (ready as unknown as { workbench: WindowStarter & { subscribe: (listener: () => void) => () => void } }).workbench
    syncWindowed()
    ready.effect(
      () => workbench.subscribe(syncWindowed),
      'ui-sidebar: workbench windowed',
    )
  })
  ...
  const injectProps = (): SidebarRootInjected => ({
    ...
    windowed: () => windowedStore.getSnapshot(),
    createWindow: (index) => {
      const workbench = ctx.get('workbench') as WindowStarter | undefined
      ...
      void workbench.createWindow(index)
    },
    toggleSidebar: () => { ctx.layout.toggleSidebar() },
    selectPanel: (id) => { ctx.layout.selectPanel(id) },
    hooks: { panels, windowed: windowedStore },
  })
