...
import { NewSessionControl } from './NewSessionControl.tsx'
...
/**
 * Sidebar-open and New Session controls in the frame's window-chrome seat. On
 * macOS desktop a collapsed sidebar hides entirely (no rail), taking both
 ...
 * placement, so the occupant renders unconditionally. New Session keeps the
 * three-window picker when windows are configured.
...
export function HeaderLeadingControls({
  toggleSidebar, startSession, windowed = () => false, createWindow = () => {},
  useWindowed, t,
}: HeaderLeadingControlsProps) {
  const windowPicker = useWindowed !== undefined ? useWindowed(value => value) : windowed()
  return (
